...
var MongoRepository = (function () {
    ...
    MongoRepository.prototype.UpdateUserInfo = function (data, callback) {
        var repo = this;
        this._collections[userCollection].findOne({
            googleId: data.id
        }, function (err, user) {
            if(err || !user) {
    ...
    MongoRepository.prototype.GetUsers = function (callback) {
        this._collections[userCollection].find({
        }, function (err, docs) {
            callback(docs);
        });
    };
